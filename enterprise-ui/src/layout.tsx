...
export function AppShell({ children, workspace }: { children: ReactNode; workspace: WorkspaceSnapshot }) {
  ...
  return <div className="app-shell">
    <aside id="app-navigation" className={`sidebar ${mobileOpen ? 'open' : ''}`} aria-hidden={!mobileOpen ? undefined : false}>
      ...
      <nav className="nav-groups">{navGroups.map((group) => <section key={group.label} className="nav-group"><div className="nav-group-label">{group.label}</div>{group.items.map(({ path, label, icon: Icon }) => {
        ...
        return <Link key={path} to={path} className={`nav-link ${active ? 'active' : ''}`}><Icon size={17} /><span>{label}</span>{path === '/issues' && openIssues > 0 && <em>{openIssues}</em>}</Link>;
      })}</section>)}</nav>
    ...
    <main className="main-shell">
      <header className="topbar">
        ...
        <div className="global-search">
          <label className="topbar-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search assets, connections, runs, issues, and monitors" aria-label="Search workspace" />{query && <button type="button" aria-label="Clear search" onClick={() => setQuery('')}><X size={14} /></button>}</label>
