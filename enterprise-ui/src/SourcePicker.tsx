...
export function SourcePicker({
  ...
  return <section className="panel form-panel">
    ...
    {sourceMode === 'manual-upload' && <label className={`dropzone ${file ? 'has-file' : ''}`}>
      ...
      <UploadCloud size={28} />
      <strong>{file ? file.name : 'Drop a CSV or .xlsx file here'}</strong>
      <span>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB · ready to profile` : 'One-time upload. The browser cannot reveal or remember the original local path.'}</span>
    </label>}
