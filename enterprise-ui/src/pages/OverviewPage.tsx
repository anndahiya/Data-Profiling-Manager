...
import { CHART_COLORS, formatCompact, latestRunFor, weightedDimensions, weightedOverallQuality } from '../utils';
...
export function OverviewPage({ workspace }: { workspace: WorkspaceSnapshot }) {
  ...
  const trend = [...workspace.runs].sort((a, b) => a.createdAt.localeCompare(b.createdAt)).slice(-12).map((run) => ({
    date: new Date(run.createdAt).toLocaleDateString('en', { month: 'short', day: 'numeric' }),
    quality: Number(run.quality.overallScore.toFixed(1)), rows: run.rowCount,
  }));
