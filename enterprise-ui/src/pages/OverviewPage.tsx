...
export function OverviewPage({ workspace }: { workspace: WorkspaceSnapshot }) {
  ...
  const governedRuns = latestRuns.filter((run) => hasGovernedQuality(run.quality));
  ...
  return <>
    ...
    <div className="metric-grid four">
      <MetricCard label="Overall quality" value={quality === undefined ? 'N/A' : `${quality.toFixed(1)}%`} detail={quality === undefined ? 'No latest runs have governed DQ results' : `${governedRuns.reduce((sum, run) => sum + run.quality.rulesEvaluated, 0)} evaluated governed rules`} tone={quality === undefined ? undefined : quality >= 95 ? 'good' : 'warning'} icon={<ShieldCheck size={16} />} />
    ...
    <div className="dashboard-grid">
      <section className="panel wide"><div className="panel-heading"><div><h2>Overall Data Quality trend</h2><p>Governed weighted overall score after each completed profiling run</p></div><button className="text-button" onClick={() => navigate('/compare')}>Compare runs <ChevronRight size={15} /></button></div><div className="chart-area"><ResponsiveContainer width="100%" height="100%"><LineChart data={trend}><CartesianGrid stroke="#edf0f5" vertical={false} /><XAxis dataKey="date" tickLine={false} axisLine={false} /><YAxis domain={[0, 100]} tickLine={false} axisLine={false} /><Tooltip /><Line type="monotone" dataKey="quality" stroke="#5b5bd6" strokeWidth={3} dot={{ r: 4, fill: '#fff', strokeWidth: 2 }} connectNulls={false} /></LineChart></ResponsiveContainer></div></section>
      ...
      <section className="panel"><div className="panel-heading"><div><h2>Quality by dimension</h2><p>Weighted latest governed score across assets</p></div></div>{dimensions.length ? <div className="chart-area small"><ResponsiveContainer width="100%" height="100%"><BarChart data={dimensions} layout="vertical" margin={{ left: 12, right: 18 }}><CartesianGrid stroke="#edf0f5" horizontal={false} /><XAxis type="number" domain={[0, 100]} hide /><YAxis type="category" dataKey="dimension" width={88} tickLine={false} axisLine={false} /><Tooltip /><Bar dataKey="score" fill="#5b5bd6" radius={[0, 6, 6, 0]} /></BarChart></ResponsiveContainer></div> : <div className="mini-empty"><ShieldCheck size={22} /><span>No governed DQ dimensions yet</span></div>}</section>
