...
export function LoadDemoButton({ afterLoad }: { afterLoad?: () => Promise<void> }) {
  ...
  const load = async () => {
    await db.transaction('rw', [db.datasets, db.runs, db.issues, db.rules, db.monitors, db.sourceHandles], async () => {
      await Promise.all([db.datasets.clear(), db.runs.clear(), db.issues.clear(), db.rules.clear(), db.monitors.clear(), db.sourceHandles.clear()]);
