...
export function SettingsPage({ workspace, reload }: { workspace: WorkspaceSnapshot; reload: () => Promise<void> }) {
  ...
  return <>
    ...
    <div className="settings-grid"><section className="panel"><div className="panel-heading"><div><h2>Privacy & storage</h2><p>This web build is local-first.</p></div><ShieldCheck size={22} className="accent-icon" /></div><ul className="check-list"><li><Check size={16} /> CSV and .xlsx files are processed in your browser.</li><li><Check size={16} /> Raw rows are not sent to a hosted application server.</li><li><Check size={16} /> Aggregate profiles, rules, and issues are stored in IndexedDB.</li><li><Check size={16} /> Multi-user auth, server schedules, and notifications require the Cloudflare backend phase.</li></ul></section><section className="panel"><div className="panel-heading"><div><h2>Workspace data</h2><p>{workspace.datasets.length} assets · {workspace.runs.length} runs · {workspace.issues.length} issues</p></div></div><div className="stack-actions"><button className="secondary-button" onClick={exportWorkspace}><FileBarChart size={16} /> Export workspace backup</button><LoadDemoButton afterLoad={reload} /><button className="danger-button" onClick={() => setConfirming(true)}><Trash2 size={16} /> Clear local workspace</button></div></section></div>
    {confirming && <ConfirmDialog title="Clear the entire local workspace?" body="This deletes every saved asset, profile run, issue, and rule from this browser. Download a backup first if you need to keep them." confirmLabel="Clear workspace" onCancel={() => setConfirming(false)} onConfirm={() => void clear()} />}
  </>;
}
...
export function LoadDemoButton({ afterLoad }: { afterLoad?: () => Promise<void> }) {
  ...
  const load = async () => {
    await db.transaction('rw', db.datasets, db.runs, db.issues, db.rules, async () => {
      await Promise.all([db.datasets.clear(), db.runs.clear(), db.issues.clear(), db.rules.clear()]);
