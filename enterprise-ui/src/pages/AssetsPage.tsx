...
export function AssetDetailPage({ workspace }: { workspace: WorkspaceSnapshot }) {
  ...
  return <>
    ...
    {!latest ? <EmptyState title="No profile yet" body="Run the first profile to populate this asset." /> : <>
      {tab === 'Overview' && <div className="dashboard-grid"><section className="panel wide"><div className="metric-grid four embedded"><MetricCard label="Rows" value={latest.rowCount.toLocaleString()} /><MetricCard label="Columns" value={String(latest.columnCount)} /><MetricCard label="Overall quality" value={`${latest.quality.overallScore.toFixed(1)}%`} detail="Weighted rules and dimensions" /><MetricCard label="Open issues" value={String(issues.filter((issue) => issue.status === 'Open').length)} /></div><div className="panel-heading"><div><h2>Quality dimensions</h2><p>Weighted rule pass rates for contributing dimensions</p></div></div><DimensionBars dimensions={latest.quality.dimensions} /></section><section className="panel"><div className="panel-heading"><div><h2>Latest run</h2><p>{formatDate(latest.createdAt)}</p></div></div><dl className="detail-list"><div><dt>File</dt><dd>{latest.fileName}</dd></div><div><dt>Missing cells</dt><dd>{latest.missingCells.toLocaleString()}</dd></div><div><dt>Duplicate rows</dt><dd>{latest.duplicateRows.toLocaleString()}</dd></div><div><dt>Rules evaluated</dt><dd>{latest.quality.rulesEvaluated}</dd></div><div><dt>Strict compliance</dt><dd>{strictScore.toFixed(1)}%</dd></div></dl><Link className="secondary-button full" to={`/runs/${latest.id}`}>Open full report</Link></section></div>}
