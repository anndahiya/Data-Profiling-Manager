...
export function AssetDetailPage({ workspace }: { workspace: WorkspaceSnapshot }) {
  ...
  return <>
    ...
    {!latest ? <EmptyState title="No profile yet" body="Run the first profile to populate this asset." /> : <>
      {tab === 'Overview' && <div className="dashboard-grid"><section className="panel wide"><div className="metric-grid four embedded"><MetricCard label="Rows" value={latest.rowCount.toLocaleString()} /><MetricCard label="Columns" value={String(latest.columnCount)} /><MetricCard label="Overall quality" value={governed ? `${latest.quality.overallScore.toFixed(1)}%` : 'N/A'} detail={governed ? 'Weighted governed rules and dimensions' : 'No governed evaluation'} /><MetricCard label="Memory" value={latest.memoryUsageMB === undefined ? '—' : `${latest.memoryUsageMB.toFixed(2)} MB`} detail="Approximate profiled values" /></div><div className="panel-heading"><div><h2>Quality dimensions</h2><p>{governed ? 'Weighted rule pass rates for contributing dimensions' : 'No official DQ dimensions were evaluated'}</p></div></div>{governed ? <DimensionBars dimensions={latest.quality.dimensions} /> : <p className="muted">Add governed rules and rerun the profile to calculate an official DQ score.</p>}</section><section className="panel"><div className="panel-heading"><div><h2>Latest run</h2><p>{formatDate(latest.createdAt)}</p></div></div><dl className="detail-list"><div><dt>File</dt><dd>{latest.fileName}</dd></div><div><dt>Missing cells</dt><dd>{latest.missingCells.toLocaleString()}</dd></div><div><dt>Duplicate rows</dt><dd>{latest.duplicateRows.toLocaleString()}</dd></div><div><dt>Numeric columns</dt><dd>{latest.numericColumnCount ?? 'Not captured'}</dd></div><div><dt>Rules evaluated</dt><dd>{latest.quality.rulesEvaluated}</dd></div><div><dt>Records passing all rules</dt><dd>{governed ? `${strictScore.toFixed(1)}%` : 'N/A'}</dd></div></dl><Link className="secondary-button full" to={`/runs/${latest.id}`}>Open full report</Link></section></div>}
