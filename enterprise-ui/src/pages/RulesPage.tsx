...
import type { ProfileRun, QualityDimension, QualityRule, RuleSeverity, RuleType, WorkspaceSnapshot } from '../types';
import { CHART_COLORS, latestRunFor } from '../utils';

const RULE_LABELS: Record<RuleType, string> = {
  'not-null': 'Required value', unique: 'Unique value', type: 'Expected datatype', pattern: 'Pattern / regular expression',
  freshness: 'Fresh within N days', range: 'Numeric range', 'allowed-values': 'Allowed values',
...
export function RulesPage({ workspace, reload }: { workspace: WorkspaceSnapshot; reload: () => Promise<void> }) {
  ...
  const rules = workspace.rules.filter((rule) => datasetFilter === 'All' || rule.datasetId === datasetFilter).sort((a, b) => a.dimension.localeCompare(b.dimension) || a.name.localeCompare(b.name));
  ...
  const deleteRule = async (rule: QualityRule) => {
    if (!window.confirm(`Delete “${rule.name}”? Historical runs retain their frozen rule configuration; this affects future runs only.`)) return;
...
function DimensionEditor({ value, onCancel, onSave }: { value?: QualityDimension; onCancel: () => void; onSave: (dimension: QualityDimension) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation"><div className="modal-card small" role="dialog" aria-modal="true" aria-labelledby="dimension-editor-title"><button className="modal-close" aria-label="Close dimension editor" onClick={onCancel}><X size={18} /></button><h2 id="dimension-editor-title">{value ? `Edit ${value.name}` : 'Add a quality dimension'}</h2><p>Create an organisation-specific dimension or change how strongly a dimension contributes to overall quality.</p><label className="field"><span>Name</span><input ref={firstRef} value={name} disabled={Boolean(value && value.source !== 'User')} onChange={(event) => setName(event.target.value)} placeholder="Referential integrity" /></label><label className="field"><span>Description</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label><div className="field-grid"><label className="field"><span>Overall score weight</span><input type="number" min="0" step="0.1" value={weight} onChange={(event) => setWeight(Number(event.target.value))} /></label><label className="field"><span>Status</span><select value={enabled ? 'enabled' : 'disabled'} onChange={(event) => setEnabled(event.target.value === 'enabled')}><option value="enabled">Enabled</option><option value="disabled">Disabled</option></select></label></div><div className="modal-actions"><button className="ghost-button" onClick={onCancel}>Cancel</button><button className="primary-button" disabled={!name.trim()} onClick={submit}>Save dimension</button></div></div></div>;
}
