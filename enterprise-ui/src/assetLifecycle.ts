...
export async function deleteAssetCascade(datasetId: string): Promise<AssetCascadeCounts> {
  const [runs, issues, rules, monitors, connections, handle] = await Promise.all([
  ...
  return { runs: runs.length, issues: issues.length, rules: rules.length, monitors: monitors.length, connections: connections.length, linkedSources: handle ? 1 : 0 };
}
